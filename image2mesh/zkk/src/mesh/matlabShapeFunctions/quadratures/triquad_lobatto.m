function [X,Y,Wx,Wy]=triquad_lobatto(N,v)

%% DO NOT USE THIS BECAUSE IT COLAPSES AN QUAD EDGE TO A VERTEX, AND HENCE
%% SINCE IT  USES A CLOSED QUADRATURE YOU EVALUATE A POINT N TIMES
...
[kk,kk,xx,yy,Wx,Wy] = lglnodesQuad(N);
xx = (xx + 1)/2;
yy = (yy + 1)/2;
yy = yy.*xx;
%
a = zeros(size(xx,1)*size(xx,2),2);
count = 0;
for i =1:size(xx,1)
    for j =1:size(xx,2)
        count = count+1;
