function [p,p_xi,p_eta] = orthopoly2D_deriv_quads(x,nDeg,element)
    ...
    for ix = 1:size(p_x,1) %nDeg+1
        for iy = 1:size(p_y,1) %nDeg+1
            ...
            p_xi(count,:)     =  dp_x(ix,:).*  p_y(iy,:) ;
            p_eta(count,:)    =   p_x(ix,:).* dp_y(iy,:) ;
        end
    end  
...
% %  This is just if one wants to see with its own eyes (hand check)
% %     xx = [-1 0 1];
% %     yy = xx;
% %     X = zeros(9,2);
% %     count = 0;
