...
% almostOne = 0.999999999999999;


p = zeros(N,size(x,1));
dp_dxi  = zeros(N,size(x,1));
dp_deta = zeros(N,size(x,1));
dp_dzet = zeros(N,size(x,1));

r = x(:,1); s = x(:,2); t = x(:,3);

% facTol  = 1e-14; % PARCHE
% % arrangeEtaZeta=find(eta==-zeta) -> eta=-zeta iff s =1
% arrange=find(s==1); % PARCHE
% s(arrange)   = s(arrange)-facTol;% PARCHE 
% arrange=find(t==1);% PARCHE
% t(arrange)   = t(arrange)-facTol;% PARCHE
if(find(abs(s)==1))
...
for nDeg = 0:n
  ...
  for i = 0:nDeg
     ...
     for j = 0:(nDeg-i)
        ...
        k = nDeg-(i+j);
        if k==0
           p_k =  ones(size(x,1),1); %1;
           dp_k =  zeros(size(x,1),1); %0;
        else
           p_k = jacobiP_vect(t,2*(i+j)+2,0,k);  
           dp_k = jacobiP_vect(t,2*(i+j)+3,1,k-1)*(k+2*i+2*j+3)/2;
        end
        ncount= ncount+1;
...
if(find(isnan(dp_dxi)) )
    error('es 1!')
end

if(find(isnan(dp_deta)) )
    error('es 2!')
end

if(find(isnan(dp_dzet)) )
    error('es 3!')
end
