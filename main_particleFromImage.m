...
for k = 1:num_steps
    ...
    displacement_phages = sqrt(sum((x_current_phages - x_init_phages).^2, 1));
    abs_disp_phages(k, :) = displacement_phages;
end
mean_disp_phages = mean(abs_disp_phages, 2);
...
for i = 1:num_subdiff_particles
    x_sub = subdiff_traj(num_steps, dt, alpha, D);
